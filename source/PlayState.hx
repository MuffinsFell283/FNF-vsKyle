...
class PlayState extends MusicBeatState
{
	...
	public function endSong():Void
	{
		...
		if(achievementObj != null) {
			...
			var achieve:String = checkForAchievement(['kyle_nomiss', 'lucas_nomiss', 'lei_nomiss', 'kyleago_nomiss', 'blent_nomiss',
			'future_nomiss', 'ex_nomiss', 'ur_bad', 'ur_good', 'hype', 'two_keys', 'toastie', 'debugger']);
	...
	private function checkForAchievement(achievesToCheck:Array<String> = null):String
	{
		...
		for (i in 0...achievesToCheck.length) {
			...
			if(!Achievements.isAchievementUnlocked(achievementName) && !cpuControlled) {
				...
				{
					case 'kyle_nomiss' | 'lucas_nomiss' | 'lei_nomiss' | 'kyleago_nomiss' | 'blent_nomiss' | 'future_nomiss':
						if(isStoryMode && campaignMisses + songMisses < 1 && CoolUtil.difficultyString() == 'HARD' && storyPlaylist.length <= 1 && !changedDifficulty && !usedPractice)
						{
							...
							{
								case 'weekKyle':
									if(achievementName == 'kyle_nomiss') unlock = true;
								case 'weekLucas':
									if(achievementName == 'lucas_nomiss') unlock = true;
								case 'weekLei':
									if(achievementName == 'lei_nomiss') unlock = true;
								case 'weekKyleAgo':
									if(achievementName == 'kyleago_nomiss') unlock = true;
								case 'weekBlent':
									if(achievementName == 'blent_nomiss') unlock = true;
								case 'weekFuture':
									if(achievementName == 'future_nomiss') unlock = true;
							}
						}
					case 'ex_nomiss':
						if(Paths.formatToSongPath(SONG.song) == 'ecstasy-ex' && campaignMisses + songMisses < 1 && !usedPractice) {
							unlock = true;
						}
					case 'ur_bad':
